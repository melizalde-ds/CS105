...
public class ShoppingList4 {
    ...
    public static void saveList(Scanner sIn, ArrayList<String> shoppList, String fileName) {
        try {
            System.out.println(
                    "Are you sure you wish to overwrite the 'Shopping List.txt' file with your current list? (Y/N)");
            if ("N".equalsIgnoreCase(sIn.nextLine())) {
                System.out.println("Shopping List not saved.\n");
            } else {
                File shopL = new File(fileName);
                shopL.createNewFile();
                FileWriter listWriter = new FileWriter(shopL);
                listWriter.write(
                        "--------------------------\n" + "\tShopping List" + "\n--------------------------\n");
                for (String item : shoppList) {
                    listWriter.write(
                            item.substring(0, item.indexOf(":")) + "\t" + item.substring(item.indexOf(":") + 1) + "\n");
                }
                listWriter.write("--------------------------\n");
    ...
    public static void openList(Scanner sIn, ArrayList<String> shoppList, String fileName) {
        String temp;
        try {
            System.out.println(
                    "Are you sure you wish to overwrite your current list with with the 'Shopping List.txt' file? (Y/N)");
            if ("N".equalsIgnoreCase(sIn.nextLine())) {
                System.out.println("Shopping List not opened.\n");
            } else {
                File shopL = new File(fileName);
                Scanner fileReader = new Scanner(shopL);
                shoppList.clear();
                for (int i = 0; i < 3; i++) {
                    fileReader.nextLine();
                }
