...
public class ValidationMethods {
	...
	public static int getValidInt(Scanner sIn, String question, String warning, int[] range) {
		...
		do {
			...
			try {
				rInt = Integer.parseInt(sIn.nextLine());
				dw1 = true;
			} catch (Exception e) {
		...
		if (range.length != 0) {
			if (range[0] <= rInt && range[1] >= rInt) {
				return rInt;
			} else {
				System.out.println(warning);
				rInt = getValidInt(sIn, question, warning, range);
				return rInt;
			}
		} else {
			return rInt;
	...
	public static double getValidDouble(Scanner sIn, String question, String warning, double[] range) {
		...
		do {
			System.out.println(question);
			try {
				rDou = Double.parseDouble(sIn.nextLine());
				dw1 = true;
			} catch (Exception e) {
				System.out.println(warning);
			}
		...
		if (range.length != 0) {
			if (range[0] <= rDou && range[1] >= rDou) {
				return rDou;
			} else {
				System.out.println(warning);
				rDou = getValidDouble(sIn, question, warning, range);
				return rDou;
			}
		} else {
			return rDou;
	...
	public static String getValidString(Scanner sIn, String question, String warning, String[] choices) {
		...
		if (choices.length != 0) {
			...
			for (String input : choices) {
				if (input.equals(rStr)) {
					return rStr;
				}
			}
			System.out.println(warning);
			rStr = getValidString(sIn, question, warning, choices);
			return rStr;
		} else {
			System.out.println(question);
			rStr = sIn.nextLine();
